  ...
  const login = async (username, password) => {
    ...
    try {
      ...
      if (data && data.length > 0 && data[0].success) {
        ...
        throw new Error(`Incorrect username or password. Remaining attempts: ${3 - newAttempts}`);
      }
    } catch (err) {
      // If error already thrown, re-throw it
      if (err.message.includes('Account locked') || err.message.includes('Incorrect username or password')) {
        throw err;
      }
